...
processFasta(char *filename, double *timeTaken)
{
	...
	int maxSize = 500;
	long *data = (long *)calloc(maxSize, sizeof(long));
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			if(recordNumber >= maxSize) {
				maxSize *= 2;
				long *newData = (long *)calloc(maxSize, sizeof(long));
				if(newData == NULL) {
	...
	// find unused memory percentage
	double unused = 1 - ((double) recordNumber/maxSize);
	printf(" -- %d allocated (%.3lf%% waste)\n", maxSize, unused);
