...
processFasta(char *filename, double *timeTaken)
{
	...
	//initialize linked list with head and tail pointers
	LinkedList *dataList = (LinkedList *)malloc(sizeof(LinkedList));
	dataList->head = NULL;
	dataList->tail = NULL;
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			// add new fasta record to linked list
			LLvNode *node = llNewNode(fRecord.description, fRecord.id);
			if(node == NULL) {
				printf("Error initializing node.\n");
				return -1;
			}
			dataList->head = llAppend(dataList, node);

			fastaClearRecord(&fRecord);
		} else {
			fprintf(stderr, "status = %d\n", status);
			fprintf(stderr, "Error: failure at line %d of '%s'\n", lineNumber, filename);
			llFree(dataList->head, NULL, NULL);
			free(dataList);
			return -1;
		}
...
int
processFastaRepeatedly(char *filename, long repeatsRequested)
{
